...
def normalize_marker_map(
    markers_cfg: dict, source: BuildSource = BuildSource.NCBI
) -> dict[str, dict]:
    ...
    for key, cfg in markers_cfg.items():
        ...
        marker_codes = (
            bold_cfg.get("marker_codes") if isinstance(bold_cfg, dict) else None
        )
...
def resolve_marker_key(value: str, marker_map: dict[str, dict]) -> str:
    ...
    if exact_matches:
        if len(exact_matches) > 1:
            raise typer.BadParameter(
                f"Marker '{value}' matches multiple entries: {exact_matches}"
            )
        return exact_matches[0]
    if prefix_matches:
        if len(prefix_matches) > 1:
            raise typer.BadParameter(
                f"Marker '{value}' matches multiple entries: {prefix_matches}"
            )
