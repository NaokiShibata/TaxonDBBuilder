...
def _dump_genbank_record(
    record: Any, acc: str, taxid: str, dump_gb_dir: Path | None, lock: Lock
) -> None:
    if not dump_gb_dir:
        return
...
def _match_ncbi_feature(
    record: Any, feature: Any, marker_rules: list[dict]
) -> _MatchedNCBIFeature | None:
    matched_label = None
    matched_marker = None
    header_format = None
...
def setup_entrez(ncbi_cfg: dict, warn_if_missing: bool = True) -> None:
    ...
    if not email and warn_if_missing:
        console.print(
            "[yellow]WARNING:[/yellow] NCBI email is not set. Set ncbi.email or NCBI_EMAIL."
        )
...
def match_feature(
    feature, compiled_patterns: list[re.Pattern], fields: list[str]
) -> str | None:
...
def build_filter_terms(filters: dict) -> list[str]:
    ...
    def as_date_str(value: object, name: str) -> str | None:
        if value is None:
            return None
        if isinstance(value, str):
            return value
    ...
    if length_min is not None or length_max is not None:
        try:
            ...
            raise typer.BadParameter(
                "filters.sequence_length_min/max must be integers."
            ) from exc
        terms.append(f"{lmin}[SLEN] : {lmax}[SLEN]")

    date_from = as_date_str(
        filters.get("publication_date_from"), "publication_date_from"
    )
    ...
    date_from = as_date_str(
        filters.get("modification_date_from"), "modification_date_from"
    )
    ...
    include_keywords = as_str_list(
        filters.get("all_fields_include"), "all_fields_include"
    )
    if include_keywords:
        inc = [f'"{k}"[All Fields]' for k in include_keywords]
        terms.append("(" + " OR ".join(inc) + ")")

    exclude_keywords = as_str_list(
        filters.get("all_fields_exclude"), "all_fields_exclude"
    )
    if exclude_keywords:
...
def build_query(taxid: str, marker_query: str, filters: dict, taxon_noexp: bool) -> str:
    tax_term = (
        f"txid{taxid}[Organism:noexp]" if taxon_noexp else f"txid{taxid}[Organism]"
    )
...
def resolve_taxon(taxon: str, require_scientific_name: bool = False) -> ResolvedTaxon:
    ...
    if re.fullmatch(r"\d+", taxon):
        taxid = taxon
        scientific_name = (
            fetch_taxonomy_scientific_name(taxid) if require_scientific_name else taxon
        )
    else:
...
def _create_genbank_fetch_context(
    ...
    if rettype not in {"gb", "gbwithparts"}:
        raise typer.BadParameter(
            "ncbi.rettype must be 'gb' or 'gbwithparts' for region extraction."
        )
    handle = Entrez.esearch(
        db=db, term=query, retmax=0, usehistory="y" if use_history else "n"
    )
    record = Entrez.read(handle)
...
def _genbank_cache_path(ctx: _GenbankFetchContext, start: int) -> Path | None:
    if not ctx.cache_root:
        return None
    return ctx.cache_root / f"start{start:09d}_count{ctx.per_query:04d}.cache"


def _load_genbank_cache(ctx: _GenbankFetchContext, start: int) -> str | None:
...
def _read_efetch_with_retries(
    ctx: _GenbankFetchContext, start: int, **fetch_kwargs: Any
) -> str:
...
def _fetch_genbank_chunk_by_ids(ctx: _GenbankFetchContext, start: int) -> str | None:
...
def _iter_genbank_history_chunks(
    ctx: _GenbankFetchContext,
...
def fetch_genbank(
    ...
    ctx = _create_genbank_fetch_context(
        query, ncbi_cfg, delay_sec, start_at, dump_dir, resume, taxid
    )
    if ctx.count == 0:
        return 0, []
...
def iter_genbank_files(
    from_dir: Path, taxid: str | None = None
) -> Iterable[tuple[int, str]]:
    gb_root = from_dir
    if taxid:
