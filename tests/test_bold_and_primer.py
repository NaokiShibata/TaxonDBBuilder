...
class _BoldResponse:
    def __init__(self, body: bytes, *, encoding: str = "", content_length: str | None = None):
...
def test_bold_runtime_config_and_payload_parsing():
    ...
    cfg = bold.get_bold_runtime_config({"base_url": "https://example.invalid/", "download_format": "JSON"})
    ...
    assert bold._build_url("https://example.invalid", "/query", {"q": "a b"}) == "https://example.invalid/query?q=a+b"
...
def test_bold_request_json_retries_gzip_and_errors(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(bold.BoldApiError, match="HTTP error 500"):
        bold._request_json("https://example.invalid", bold.get_bold_runtime_config({"retries": 0}))

    monkeypatch.setattr(bold, "urlopen", lambda *args, **kwargs: _BoldResponse(b"not-json"))
...
def test_bold_query_preparation_and_validation(monkeypatch: pytest.MonkeyPatch):
    ...
    responses = iter([
        {"successful_terms": [{"scope": "taxon", "field": "name", "value": "Testus"}, "marker"]},
        {"counts": {"specimens": [2]}},
        {"queryId": "query 1"},
    ])
    monkeypatch.setattr(bold, "_request_json", lambda *_args, **_kwargs: next(responses))
    ...
    assert bold.fetch_specimen_count("x", {"base_url": "x", "retries": 0, "timeout_sec": 1, "backoff_sec": 0, "user_agent": "x", "download_chunk_size": 1}) is None
    ...
    monkeypatch.setattr(bold, "_request_json", lambda *_args, **_kwargs: {"count": 2_000_000})
    ...
    with pytest.raises(bold.BoldApiError, match="query_id"):
        bold.submit_query("x", {"base_url": "x", "retries": 0, "timeout_sec": 1, "backoff_sec": 0, "user_agent": "x", "download_chunk_size": 1})


def test_bold_download_and_document_row_iterators(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    import taxondb_bold as bold

    payload = b"marker_code\tnucleotides\nCOI-5P\tAACCGG\n"
    monkeypatch.setattr(bold, "urlopen", lambda *args, **kwargs: _BoldResponse(payload, content_length="bad"))
    monkeypatch.setattr(bold.time, "sleep", lambda _seconds: None)
    progress: list[tuple[int, int | None]] = []
    destination = tmp_path / "nested" / "documents.tsv"
    runtime_cfg = bold.get_bold_runtime_config({"download_chunk_size": 3, "backoff_sec": 0})
    meta = bold.download_documents_to_path("query/id", runtime_cfg, destination, progress_callback=lambda n, total: progress.append((n, total)))
    assert meta["downloaded_bytes"] == len(payload)
    assert meta["content_length"] is None
    assert progress[-1][0] == len(payload)
    assert list(bold.iter_document_rows_from_path(destination, "tsv"))[0]["marker_code"] == "COI-5P"

    json_path = tmp_path / "documents.json"
    json_path.write_text(json.dumps({"records": [{"marker_code": "COI-5P", "nucleotides": "AA"}]}), encoding="utf-8")
    assert list(bold.iter_document_rows_from_path(json_path, "json"))[0]["nucleotides"] == "AA"
    ...
    monkeypatch.setattr(bold, "urlopen", lambda *args, **kwargs: (_ for _ in ()).throw(URLError("offline")))
    with pytest.raises(bold.BoldApiError, match="network error"):
        bold.download_documents_to_path("query", bold.get_bold_runtime_config({"retries": 0, "backoff_sec": 0}), destination)
...
def test_fetch_bold_records_for_taxon_json_and_zero(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(bold, "download_documents", lambda *_args, **_kwargs: {"documents": [{
        "marker_code": "COI-5P", "nucleotides": "acgt-", "processid": "P1", "species": "Testus alpha"
    }]})
    ...
    zero = prepared.__class__(**{**prepared.__dict__, "specimen_count": 0, "query_id": None})
    ...
    monkeypatch.setattr(bold, "prepare_bold_query", lambda *_args, **_kwargs: missing_id)
    with pytest.raises(bold.BoldApiError, match="query_id"):
